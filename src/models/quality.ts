...
export function loadRoutingMetrics(
  ...
  return models.map((model) => {
    ...
    const brierSum = predictions.reduce(
      (sum, prediction) =>
        sum + (prediction.probability - prediction.resolved_label) ** 2,
      0,
    );
    const attempts = db.prepare<[string, string, string], AttemptMetricRow>(`
    ...
    const latencies = attempts === undefined
      ? []
      : z.array(z.number().int().nonnegative()).parse(
          JSON.parse(attempts.latency_samples_json),
        ).slice(-50).sort((left, right) => left - right);
    return {
      ...
      brierSum,
      p75LatencyMs: latencies.length === 0
