...
async function verifiedSupervisor(
  ...
  const identity = await provider.identify(
    row.pid,
    process.platform === "win32" ? undefined : row.pid,
  );
...
export async function ensureSupervisor(options: EnsureSupervisorOptions): Promise<void> {
  ...
  if (existing !== undefined) {
    ...
    if (
      !matching &&
      activeExecutorJobs(options.db, existing.build_id, existing.config_digest) > 0
    ) {
      throw new AppError(
  ...
  const supervisorEntrypoint =
    options.supervisorEntrypoint ?? fileURLToPath(new URL("./supervisor-main.js", import.meta.url));
  const workerEntrypoint =
    options.workerEntrypoint ?? fileURLToPath(new URL("./worker-main.js", import.meta.url));
  const child = spawn(
    process.execPath,
    [
  ...
  options.db
    .prepare("DELETE FROM supervisor_state WHERE singleton = 1 AND owner_token = ?")
    .run(ownerToken);
